...
class Manage_report extends CI_Controller
{
	...
	function listing($offset=0,$order='emp_id',$srt='desc'){
		...
		if(isset($whr) and $whr!='')
			...
			$total_rows       = $this->manage_report_model->rows($whr,$whr1);

		else

			$total_rows       = $this->manage_report_model->rows($whr=1,$whr1=1);
	    ...
	    $data['result'] = $this->manage_report_model->get_where($table,$whr,false,"$order $srt",true,$config['per_page'],$offset,$whr1);
	...
	////////for ordering///////////
	    ...
	    $data['result'] = $this->manage_report_model->get_employee_detail_where($whr,$offset,$id,$srch_date,$role);
		$emp_name= $this->manage_report_model->employee_name($id,$role); 
	...
	public function pdf_report(){
	 ...
	 $res= $this->manage_report_model->get_pdf_emp_result($id,$role,$srch_date); 
