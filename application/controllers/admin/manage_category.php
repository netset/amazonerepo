...
class Manage_category extends CI_Controller
{
	...
	function listing($offset=0,$order='id',$srt='desc'){
		...
		if(isset($whr) and $whr!='')
			...
			$total_rows       = $this->manage_category_model->rows($whr);

		else

			$total_rows       = $this->manage_category_model->rows($whr=1);
        ...
        $config['base_url']       = base_url().'admin/manage_category/listing';
	    ...
	    $data['result'] = $this->manage_category_model->get_where($table,$whr,false,"$order $srt",true,$config['per_page'],$offset);

		$data['file']="admin/category/listing";	
	...
	function listing_search($offset=0,$order='id',$srt='desc'){
        ...
        $config['base_url']       = base_url().'admin/manage_category/listing_search';
 ...
 function get_city()
 {
  ...
  //pr($data);die;
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['file']       = "admin/category/add_category";
                ...
                if ($id != '') {
                    ...
                    if ($this->manage_category_model->update_category($id)) {
                        $this->session->set_flashdata('status', '<div class="msg">' . 'Category has been Updated Successfully' . '</div>');
                    } else {
                        $this->session->set_flashdata('status', '<div class="error">' . 'Problem Updating' . '</div>');
                    }
                } else {
                  if ($this->manage_category_model->add_category()) {
                        $this->session->set_flashdata('status', '<div class="msg">' . 'category has been added Successfully' . '</div>');
                       
                    } else {
                        $this->session->set_flashdata('status', '<div class="error">' . 'Problem Adding' . '</div>');
                    }
                }
                redirect(base_url() . "admin/manage_category/listing");
            }
            
        } else {
            if ($id != '') {
                
                $data['result'] = $this->manage_category_model->get_category($id);
	...
	/*function edit_employee($id=''){
		...
		$data['countries']=$this->manage_category_model->get_all_countries();				
		$data['departments']=$this->manage_category_model->get_all_departments();
		if($this->input->post()){				
			/* form validation 
			$this->load->library('form_validation');
			...
			if($this->form_validation->run() == FALSE){
				...
				if($id!=''){
						if($this->manage_category_model->update_user($id)){
						...
						if($this->manage_category_model->add_user($data)){					
			...
			if($id!=''){			
				$data['result']=$this->manage_category_model->get_employee($id);				
			}
	...
	*/
	function detailed_hours($id,$offset=0,$order='id',$srt='desc')
		...
		$table = '';
 	    ...
 	    $data['offset']       = 0;
      	...
      	$data['result'] = $this->manage_category_model->get_employee_detail_where($whr,$offset,$id,$data['datepicker'],$data['datepicker1']);
	...
	public function download_detailed_hours(){
		...
		$full_name= $emp_name[0]->fname ." ". $emp_name[0]->lname;
      	$res = $this->manage_category_model->download_get_employee_detail_where($id,$srch_date);
		if(strpos($srch_date,"."))
		{
...
<center><h1>'.$full_name.'</h1></center>
  ...
  <table cellspacing="0" class="admin_table">
	...
	function delete($id='')
	{
		...
		$this->manage_category_model->doAction('job_category');
	}
}
?>