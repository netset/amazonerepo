...
<div id="global_content"> 
 ...
 <?php if($this->session->flashdata('status')!=''){ ?>
              <p><?php echo $this->session->flashdata('status'); ?></p>  
                <?php } ?> 
 <form action="<?php echo base_url().'admin/manage_new_report/listing';?>" method="post" class="global_form_box">
...
<a class="category_btn" href='<?php echo base_url().'admin/manage_new_report/listing'; ?>'><?php echo 'Reset';?></a> 
</div>
...
?>
 <?php
  ...
  <a href="<?php echo base_url().'admin/manage_new_report/download_pdf_report/'.$download_search_value; ?>">  <img src="<?php echo base_url().'public/img/pdf.png' ?>" style="width:35px;" title="Pdf" alt="Pdf"></a> 
  <a href="<?php echo base_url().'admin/manage_new_report/download_csv_report/'.$download_search_value; ?>">  <img src="<?php echo base_url().'public/img/csv.png' ?>" style="width:35px;" title="Csv" alt="Csv"></a> 
  <a href="#" onclick="javascript:CallPrint('global_content_wrapper')">  <img src="<?php echo base_url().'public/img/print.png' ?>" style="width:40px;" title="Print" alt="Print"></a>
  ...
  <table cellspacing="0" class='admin_table'>
    <thead>
      <tr>
        <th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="javascript:void(0)" style='color:#000;'><?php } echo 'Name';?></a>
        </th>
		<th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="javascript:void(0)" style='color:#000;'><?php } echo 'Designation';?></a>
        </th>
...
<th width="10%" class='admin_table_centered' style='width: 3%;'><?php if(!empty($result)){?><a href="javascript:void(0)"><?php } echo 'Action';?></a></th>
  ...
  if($result){
	foreach($result as $value){?>
            ...
            <td class='admin_table_user'><?php echo htmlspecialchars($value->fname)." ".$value->lname; ?></td>
            ...
            <td class='admin_table_options'>
				<a class="fancybox fancybox.ajax" href="<?php echo base_url().'admin/manage_new_report/employee_detail/'.$value->emp_id.'/'.$value->role.'/'.@$search_value?>" ><button type="button">Detail</button></a></td>         
		 </tr>
               <?php }
}
else{
