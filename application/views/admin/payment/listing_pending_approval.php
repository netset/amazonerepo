...
<div id="global_content"> 
 ...
 <?php if($this->session->flashdata('status')!=''){ ?>
              <p><?php echo $this->session->flashdata('status'); ?></p>  
                <?php } ?> 
 <form action="<?php echo base_url().'admin/manage_payment/view_pending_jobs_for_approval';?>" method="post" class="global_form_box">
...
<a href='<?php echo base_url().'admin/manage_payment/view_pending_jobs_for_approval'; ?>' class="category_btn"><?php echo 'Reset';?></a> 
<a href="<?php echo base_url().'admin/manage_payment/view_approved_jobs'?>" class="category_btn"><?php echo 'Approved Jobs';?></a></div>
  ...
  <table cellspacing="0" class='admin_table'>
    <thead>
      <tr>
        ...
        <th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="<?php echo base_url().'admin/manage_payment/view_pending_jobs_for_approval/'.$offset.'/job.job_num/'.$pasc?>" style='color:#000;'><?php } echo 'Job Number';?></a>
			
		<th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="<?php echo base_url().'admin/manage_payment/view_pending_jobs_for_approval/'.$offset.'/emp.fname/'.$pasc?>" style='color:#000;'><?php } echo 'Job Assign To';?></a>
        </th>
      ...
      <th width="10%" class='admin_table_centered' style='width: 3%;'><?php if(!empty($result)){?><a href="javascript:void(0)"><?php } echo 'Approve';?></a></th>
  ...
  if($result){
	foreach($result as $value){?>
            ...
            <td class='admin_table_options'>
                          <a href="<?php echo base_url().'admin/manage_payment/delete/'.$value->id?>" title="Delete" onclick="return confirm('Are you sure you want to delete ?');"><img src="<?php echo base_url().'public/images/b_drop.png'?>" alt="Delete"></a>&nbsp;&nbsp;

							<a href="<?php echo base_url().'admin/manage_payment/add_job/'.$value->id?>" ><img src="<?php echo base_url().'public/images/edit.png'?>" alt="Edit" title="Edit Job"></a></td>
								
            <td class='admin_table_options'>
&nbsp;&nbsp;

							<a href="<?php echo base_url().'admin/manage_payment/dicision/'.$value->id?>" class="fancybox fancybox.ajax"><img src="<?php echo base_url().'public/images/dicision.png'?>" style="width:25px;" alt="Approve" title="Approve Job"></a></td>
...
<tr><td colspan='4'><?php echo "No such record found";
  ...
  <input type='hidden' id='action' name='action' value='' />
    <button type='submit' value="Approve Selected" onclick='return changeAction("approve");'><?php echo 'Approve Selected';?></button>
