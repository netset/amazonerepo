...
<div id="global_content"> 
 ...
 <?php if($this->session->flashdata('status')!=''){ ?>
              <p><?php echo $this->session->flashdata('status'); ?></p>  
                <?php } ?> 
 <form action="<?php echo base_url().'admin/manage_payment/view_completed_jobs';?>" method="post" class="global_form_box">
...
<a href='<?php echo base_url().'admin/manage_payment/view_completed_jobs'; ?>' class="category_btn"><?php echo 'Reset';?></a> 
  ...
  <table cellspacing="0" class='admin_table'>
    <thead>
      <tr>
        ...
        <th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="<?php echo base_url().'admin/manage_payment/view_completed_jobs/'.$offset.'/job.job_num/'.$pasc?>" style='color:#000;'><?php } echo 'Job Number';?></a>
		<th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="<?php echo base_url().'admin/manage_payment/view_completed_jobs/'.$offset.'/job.job_descr/'.$pasc?>" style='color:#000;'><?php } echo 'Job Description';?></a>
        </th>		
		<th width="10%" style='width: 1%;'><?php if(!empty($result)){?><a href="<?php echo base_url().'admin/manage_payment/view_completed_jobs/'.$offset.'/emp.fname/'.$pasc?>" style='color:#000;'><?php } echo 'Job Assign To';?></a>
        </th>
  ...
  if($result){
	foreach($result as $value){?>
            ...
            <td class='admin_table_user'>#<?php echo ucfirst(htmlspecialchars($value->job_num)); ?></td>
             ...
             <td class="admin_table_centered nowrap"><?php echo ucfirst(substr("$value->job_descr",0,50)),"...."; ?>  
            </td>	             
			<td class="admin_table_centered nowrap"><?php echo $value->fname; ?>  
            </td>		
            <td class='admin_table_centered'>
       	
			<?php if($value->payment_approval_status==0){ echo "Pending";}elseif($value->payment_approval_status==1){ echo "Approved";}else{echo "Rejected";}?>
			</td>
